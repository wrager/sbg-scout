 ...
 * Ветка `onNewIntent` (deep-link в уже живой инстанс с `launchMode=singleTop`)
 * не покрывается e2e из-за ограничений `ActivityScenario`: после
 * `callActivityOnNewIntent` lifecycle-трекинг scenario не даёт закрыть
 * Activity в `tearDown` (Activity застревает в RESUMED до таймаута 45с).
 * Сама реализация `onNewIntent` — 5 строк, делегирующих `extractDeepLinkUrl`
 * + `webView.loadUrl`, те же функции покрыты через `onCreate`-путь.
 */
